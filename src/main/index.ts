...
app.whenReady().then(()=>{useSystemCertificateAuthorities(session.defaultSession);session.defaultSession.webRequest.onErrorOccurred(details=>console.error('[Request failed]',{url:details.url,error:details.error}));repository=new JsonDownloadRepository(join(app.getPath('userData'),'downloads.json'));let notify=()=>{},finished=()=>{};const engine=new ElectronDownloadEngine(session.defaultSession,repository,()=>notify(),()=>finished());service=new DownloadService(repository,engine,()=>notify());notify=()=>broadcastDownloads(service);finished=()=>service.onDownloadFinished();registerDownloadHandlers(service);createWindow();const startupUrl=process.argv.find(value=>/^https?:\/\//i.test(value));if(startupUrl)setTimeout(()=>service.add(startupUrl),500);app.on('activate',()=>{if(BrowserWindow.getAllWindows().length===0)createWindow()})})
