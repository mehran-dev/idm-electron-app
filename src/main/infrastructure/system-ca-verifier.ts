...
function loadSystemAuthorities():X509Certificate[]{
  ...
  return pem.match(/-----BEGIN CERTIFICATE-----[\s\S]+?-----END CERTIFICATE-----/g)?.flatMap(value=>{
    try{return[new X509Certificate(value)]}catch{return[]}
  })??[]
}
...
export function useSystemCertificateAuthorities(targetSession:Session):void{
  ...
  targetSession.setCertificateVerifyProc((request,callback)=>{
    if(request.verificationResult==='net::OK'){callback(-3);return}
    try{
      ...
      const trusted=valid&&hostMatches&&authorities.some(authority=>leaf.verify(authority.publicKey))
      if(trusted){
        console.info('[TLS accepted by operating-system CA]',{hostname:request.hostname,issuer:leaf.issuer})
        callback(0)
        return
      }
    }catch(error){console.error('[TLS fallback verification failed]',error)}
    console.error('[TLS certificate rejected]',{hostname:request.hostname,error:request.verificationResult,issuer:request.certificate.issuerName})
    callback(-3)
  })
}
