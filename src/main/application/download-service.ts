...
export class DownloadService{
  ...
  add(urlValue:string,segmentCount?:number){const item=this.create(urlValue,segmentCount);this.engine.start(item.id,item.url,item.fileName);return item}
  enqueue=(urlValue:string,queueId='main',segmentCount?:number)=>{const item=this.create(urlValue,segmentCount);const queue=this.repo.getQueue(queueId)??this.repo.allQueues()[0];item.queueId=queue?.id;this.repo.save(item);this.notify();return item}
  ...
  removeFromList=(id:string)=>{if(this.engine.isActive(id))this.engine.cancel(id);this.repo.remove(id);this.notify()}
  ...
  private create(urlValue:string,segmentCount?:number):DownloadItem{const url=new URL(urlValue);if(!['http:','https:'].includes(url.protocol))throw new Error('Only HTTP and HTTPS URLs are supported.');const fileName=decodeURIComponent(basename(url.pathname))||`download-${Date.now()}`;const chosen=[1,2,4,6,8].includes(segmentCount??0)?segmentCount!:this.repo.getSegmentCount();const item:DownloadItem={id:randomUUID(),url:url.href,fileName,savePath:'',status:'queued',receivedBytes:0,totalBytes:0,speed:0,segmentCount:chosen,createdAt:new Date().toISOString()};this.repo.save(item);this.notify();return item}
